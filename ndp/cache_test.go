...
// TestNDPCache_Stop_CancelsGoroutines verifies that Stop() properly
// cancels all spawned goroutines to prevent goroutine leaks.
func TestNDPCache_Stop_CancelsGoroutines(t *testing.T) {
...
// TestNDPCache_ContextCancellation verifies that context cancellation
// properly stops goroutines from spawning new update tasks.
func TestNDPCache_ContextCancellation(t *testing.T) {
	c := newCache()
...
// TestNDPCache_NoGoroutineLeaks tests that multiple Start/Stop cycles
// don't cause goroutine leaks.
func TestNDPCache_NoGoroutineLeaks(t *testing.T) {
	initialGoroutines := runtime.NumGoroutine()
...
// TestNDPCache_ConcurrentAccess tests that concurrent get() calls
// are safe and don't cause data races.
// Run with: go test -race
func TestNDPCache_ConcurrentAccess(t *testing.T) {
	c := newCache()
	defer c.Stop()
	...
	for i := 0; i < numGoroutines; i++ {
		go func() {
			...
			for j := 0; j < numIterations; j++ {
				...
				// Use the table
				if table != nil {
					_ = table.SearchMAC(net.ParseIP("fe80::1"))
				}
...
// TestNDPCache_GlobalStop tests the global Stop() function
func TestNDPCache_GlobalStop(t *testing.T) {
	initialGoroutines := runtime.NumGoroutine()

	// Use global cache
	for i := 0; i < 5; i++ {
		_ = SearchMAC(net.ParseIP("fe80::1"))
		time.Sleep(50 * time.Millisecond)
	}
...
// TestNDPCache_UpdateThrottling verifies that updates are properly
// throttled (30 second interval) and don't spam goroutines.
func TestNDPCache_UpdateThrottling(t *testing.T) {
	c := newCache()
	defer c.Stop()
...
// TestNDPCache_StopMultipleTimes verifies that calling Stop() multiple
// times doesn't cause panics or issues.
func TestNDPCache_StopMultipleTimes(t *testing.T) {
	c := newCache()
...
// TestNDPCache_NilContext tests behavior when context is nil
func TestNDPCache_NilContext(t *testing.T) {
	c := &cache{
		ctx:    nil,
		cancel: nil,
	}
...
// TestNDPCache_IPv6Addresses tests cache with IPv6 addresses
func TestNDPCache_IPv6Addresses(t *testing.T) {
	c := newCache()
	defer c.Stop()
	...
	for _, ipStr := range testIPs {
		...
		table := c.get()
		if table != nil {
			_ = table.SearchMAC(ip)
		}
	}
}

// TestNDPCache_StopDuringRefreshDropsLateStore verifies that a refresh
// already in flight when Stop() is called does not store its result after
// the cache has been stopped.
func TestNDPCache_StopDuringRefreshDropsLateStore(t *testing.T) {
	// Arrange
	prev := getTable
