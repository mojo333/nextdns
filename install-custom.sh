...
# Custom NextDNS Installer - Points to Your Repository
#
# Configuration: Set these environment variables before running:
#   CUSTOM_GITHUB_REPO - Your GitHub repo (e.g., "yourusername/nextdns")
#   CUSTOM_REPO_URL    - Your binary repo URL (optional, for packages)
#
# Usage:
...
install_bin() {
    ...
    */*)
        # Snapshot - use custom repo if available
        branch=${INSTALL_RELEASE%/*}
        hash=${INSTALL_RELEASE#*/}
        if [ "$CUSTOM_GITHUB_REPO" ]; then
            url="https://github.com/${GITHUB_REPO}/releases/download/snapshot-${branch}-${hash}/nextdns_${GOOS}_${GOARCH}.tar.gz"
        else
            url="https://snapshot.nextdns.io/${branch}/nextdns-${hash}_${GOOS}_${GOARCH}.tar.gz"
        fi
        ;;
    *)
        # CUSTOM: Use your GitHub repo for releases
        url="https://github.com/${GITHUB_REPO}/releases/download/v${INSTALL_RELEASE}/nextdns_${INSTALL_RELEASE}_${GOOS}_${GOARCH}.tar.gz"
        ;;
    esac
...
install_rpm() {
    asroot curl -Ls ${REPO_BASE_URL}/nextdns.repo -o /etc/yum.repos.d/nextdns.repo &&
...
install_zypper() {
    if asroot zypper repos | grep -q nextdns >/dev/null; then
        ...
        asroot zypper ar -f -r ${REPO_BASE_URL}/nextdns.repo nextdns
    fi
...
install_deb_keyring() {
    ...
    ( asroot wget -qO /etc/apt/keyrings/nextdns.gpg ${REPO_BASE_URL}/nextdns.gpg ||
      asroot curl -sfL ${REPO_BASE_URL}/nextdns.gpg -o /etc/apt/keyrings/nextdns.gpg ) &&
...
install_apk() {
    repo=${REPO_BASE_URL}/apk
    asroot wget -O /etc/apk/keys/nextdns.pub ${REPO_BASE_URL}/nextdns.pub &&
...
install_brew() {
    # Note: Homebrew tap requires official repo, binary install as fallback
    if [ "$CUSTOM_GITHUB_REPO" != "nextdns/nextdns" ]; then
        log_warn "Custom repo detected, using binary install instead of Homebrew"
        install_bin
    else
...
ubios_install_source() {
    echo "deb [signed-by=/etc/apt/keyrings/nextdns.gpg] ${REPO_BASE_URL}/deb stable main" > /data/nextdns.list
    ...
    podman exec unifi-os curl -sfL ${REPO_BASE_URL}/nextdns.gpg -o /etc/apt/keyrings/nextdns.gpg
...
install_type() {
    ...
    */*)
    ...
    case $OS in
    centos|fedora|rhel)
        # Use binary install if custom repo (no RPM repo available)
        if [ "$CUSTOM_GITHUB_REPO" != "nextdns/nextdns" ]; then
            echo "bin"
        else
            echo "rpm"
...
get_release() {
    if [ "$NEXTDNS_VERSION" ]; then
        ...
        *)
        ...
        # CUSTOM: Use your GitHub repo for latest release
        v=$($cmd "https://api.github.com/repos/${GITHUB_REPO}/releases/latest" | \
